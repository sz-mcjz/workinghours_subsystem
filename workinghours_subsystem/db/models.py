...
class WorkerInfo(models.Model):
    ...
    project_name = models.ForeignKey('Project', on_delete=models.DO_NOTHING, related_name='info_project_id')  # 所属项目
    worker = models.CharField(max_length=100)  # 工人姓名
    id_number = models.CharField(max_length=30, unique=True)  # 身份证号码
    ...
    leave_data = models.DateField(null=True)  # 立场时间
...
# 工人信息修改表
class WorkerInfoChange(models.Model):
    record_id = models.AutoField(primary_key=True)  # 记录id
    worker_info_id = models.ForeignKey("WorkerInfo", on_delete=models.CASCADE, related_name='info_change_worker_id')  # 员工信息关联
    change_date = models.DateField(auto_now_add=True)  # 修改日期
    column = models.CharField(max_length=30)  # 字段
...
class WorkerHours(models.Model):
    ...
    worker_info_id = models.ForeignKey("WorkerInfo", on_delete=models.CASCADE, related_name='worker_hours_worker_id')  # 工人信息关联
    entry_data = models.DateField(auto_now=True)  # 入场时间
    writer = models.CharField(max_length=30)  # 填报人
...
class WorkerHoursChange(models.Model):
    ...
    worker_info_id = models.ForeignKey("WorkerInfo", on_delete=models.CASCADE, related_name='record_worker_id')  # 工人信息关联
